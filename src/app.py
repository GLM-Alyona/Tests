...
def move_doc_to_shelf(user_doc_number=None, user_shelf_number=None):
    ...
    return 'Документ номер "{}" был перемещен на полку номер "{}"'.format(
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            for document_info in show_all_docs_info():
                print(document_info)
        elif user_command == 's':
